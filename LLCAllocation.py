...
class LLCAllocation:
    ...
    def threadLevelLPSolver(self, taskSystem, maxHalfWays, overheads, complex, corePerComplex, dedicatedIRQ=False, dedicatedIRQCore=None):
        '''
        ...
        for taskLevel in range(Constants.LEVEL_A, Constants.LEVEL_C+1):
            for costLevel in range(taskLevel, Constants.LEVEL_C+1):
                if taskLevel <= Constants.LEVEL_B:
                    Udata[(taskLevel,costLevel,SOLO)] = pd.DataFrame(np.zeros((corePerComplex, maxHalfWays + 1)))
                    Udata[(taskLevel,costLevel,PAIRED)] = pd.DataFrame(np.zeros((corePerComplex, maxHalfWays + 1)))
                else:
                    Udata[(taskLevel, costLevel)] = pd.DataFrame(np.zeros((len(complex.clusterList), maxHalfWays + 1)))

        h = pd.DataFrame(np.zeros((len(complex.clusterList), maxHalfWays + 1)))
        H = pd.DataFrame(np.zeros((len(complex.clusterList), maxHalfWays + 1)))

        for numHalfWays in range(0, maxHalfWays+1):
            for core in complex.coreList:
                for taskLevel in (Constants.LEVEL_A, Constants.LEVEL_B):
                    #we first generate data for paird utils
                    #solo tasks can produce higher cpmd cost
                    inflatedUtil = overheads.accountOverheadCore(taskLevel=taskLevel, allCritLevels=taskSystem.levels,
                                                 core=core, cacheSize=[numHalfWays/2,numHalfWays/2], scheme=Constants.THREAD_LEVEL_ISOLATION,
                    ...
                    for costLevel in range(taskLevel, Constants.LEVEL_C+1):
                        ...
                        Udata[(taskLevel, costLevel, SOLO)][coreIDtoIndex[core.coreID]][numHalfWays] = soloUtil
                        Udata[(taskLevel, costLevel, PAIRED)][coreIDtoIndex[core.coreID]][numHalfWays] = pairedUtil
            ...
            for cluster in complex.clusterList:
                ...
                Udata[(Constants.LEVEL_C, Constants.LEVEL_C)][clusterIDtoIndex[cluster.clusterID]][numHalfWays] = util
        return

    def coreLevelLPSolver(self, taskSystem, maxWays, overheads, complex, corePerComplex, dedicatedIRQ=False, dedicatedIRQCore=None):
        '''
        ...
        if solver.status == GRB.OPTIMAL:
            i = 0
            for core in complex.coreList:
                size = W[i].x #size has num of ways
